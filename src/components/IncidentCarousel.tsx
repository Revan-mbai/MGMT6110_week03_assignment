...
export const IncidentCarousel: React.FC<IncidentCarouselProps> = ({ incidents }) => {
  ...
  useEffect(() => {
    // Only auto-play if user is not viewing details modal
  ...
  return (
    ...
    >
      <div className="max-w-xl mx-auto">
        ...
        {/* Incident Card - low profile, thin & compact and CLICKABLE to read more */}
        <div
          ...
          title="Click to read detailed traffic advisory"
          className="bg-white rounded-lg px-2.5 py-2 shadow-2xs border border-slate-200 hover:border-amber-400 hover:shadow-xs transition-all space-y-1 cursor-pointer group text-left block w-full focus:outline-hidden focus:ring-2 focus:ring-amber-500/50"
        >
          ...
          <div className="flex items-center justify-between gap-1.5 min-w-0">
            ...
            <div className="flex items-center gap-1 text-[10px] text-slate-400 font-medium whitespace-nowrap shrink-0">
              <span className="text-amber-700 font-semibold group-hover:underline hidden xs:inline flex items-center">
                Details <ChevronRight className="w-2.5 h-2.5 inline ml-0.5" />
              </span>
